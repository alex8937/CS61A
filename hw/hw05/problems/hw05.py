...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert not lower_bound(y) <= 0 <= upper_bound(y), '0 should not be included in y'
...
def quadratic(x, a, b, c):
    ...
    if a == 0 or -b / (2 * a) < lower or -b / (2 * a) > upper:
        return interval(fn(lower), fn(upper))
    else:
        return interval((4 * a * c - b ** 2) / (4 * a ** 2), max(fn(lower), fn(upper)))
