...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        current = dice()
        if not one_occurred and current == 1:
            one_occurred = True
            total = 0
        if not one_occurred:
            total += current
...
def is_prime(num):
    assert num >= 0, 'is_prime argument must be non-negative'
    if num == 1 or num == 0:
        return False
    else:
        div = 2
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    score = prime_promotion(score)
    score = min(score, 25 - num_rolls)
    return score
...
def reroll(dice):
    ...
    def rerolled():
        current = dice()
        if not current & 1: # Check if current is even
            return current
        else:
            return dice()
        # END PROBLEM 3
...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    while score0 < goal and score1 < goal:
        ...
        my_num_rolls = strategy(player_score, opponent_score)
        dice = select_dice(player_score, opponent_score, dice_swapped)
        if my_num_rolls == -1:
            earn_score = 1
            dice_swapped = not dice_swapped
        else:
            earn_score = take_turn(my_num_rolls, opponent_score, dice)
        player_score += earn_score
        if player == 0:
            score0, score1 = swine_swap(player_score, opponent_score)
        else:
            score0, score1 = swine_swap(opponent_score, player_score)
        player = other(player)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    for new_roll in range(1, 11):
        new_average_score = average_roll_dice(new_roll, dice)
        if new_average_score > max_average_score:
            max_roll = new_roll
            max_average_score = new_average_score
    return max_roll
    # END PROBLEM 8
...
def run_experiments():
    ...
    if False:  # Change to True to test always_roll(8)
        for i in range(1, 11):
            print('always_roll({0}) win rate:'.format(i), average_win_rate(always_roll(i)))
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    # BEGIN PROBLEM 10
    gain = prime_promotion(free_bacon(opponent_score))
    if (score + gain) * 2 == opponent_score or gain >= margin:
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
