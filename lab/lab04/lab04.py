...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    dummy = make_city('dummy', lat, lon)
    distance1 = distance(dummy, city1)
    distance2 = distance(dummy, city2)
...
def put_piece(board, max_rows, column, player):
    ...
    for row in range(max_rows + 1):
        if row == max_rows or get_piece(board, row, column) != '-':
            row_index = row - 1
            break
    if row_index != -1:
        new_row = replace_elem(board[row_index], column, player)
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    -1
    """
    if col < 0 or col >= max_cols or get_piece(board, 0, col) != '-':
        return -1, board
    else:
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        ...
        for col in range(max_cols):
            string += get_piece(board, row, col) + ' '
        print(string.strip())
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    diagonal_win = check_win_diagonal(board, max_rows, max_cols, num_connect,
                                      row, col, player)
    "*** YOUR CODE HERE ***"
    return check_win_row(board, max_rows, max_cols, num_connect, row, player) \
